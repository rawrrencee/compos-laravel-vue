...
const currentRouteName = computed(() => {
  return props?.secondaryNavigation?.find((n) => n.href === props.currentRoute)?.name;
});
const breadcrumbParent = computed(() => {
  return props?.secondaryNavigation?.find((n) => n.children?.some((c) => c.href === props.currentRoute));
});
    ...
    >
      <nav class="flex" aria-label="Breadcrumb">
        <ol role="list" class="flex items-center space-x-4">
          ...
          <li>
            <div class="flex items-center">
              <ChevronRightIcon class="h-5 w-5 flex-shrink-0 text-secondary" aria-hidden="true" />
              <span class="ml-4 text-primary">{{ breadcrumbChild.name }}</span>
            </div>
          </li>
        </ol>
      </nav>
      ...
      <div class="hidden md:flex overflow-x-auto border-b border-white/10 pb-4">
        ...
        >
          <li v-for="item in secondaryNavigation" :key="item.name">
            <Link
              :href="route(item.href)"
              :class="[isCurrent(item, currentRoute, true) ? 'text-primary' : '', 'hover:text-secondary']"
              >{{ item.name }}</Link
            >
          </li>
        </ul>
